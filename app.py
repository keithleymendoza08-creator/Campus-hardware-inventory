...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'nu-lab-final-english-2026'
...
def migrate_users_table():
    try:
        ...
        if 'course' not in cols:
            cur.execute("ALTER TABLE users ADD COLUMN course TEXT DEFAULT ''")
        if 'section' not in cols:
            cur.execute("ALTER TABLE users ADD COLUMN section TEXT DEFAULT ''")
...
def dashboard():
    ...
    hardwares = db.execute("SELECT * FROM hardware WHERE name LIKE? ORDER BY id DESC", (f"%{q}%",)).fetchall()
    total = db.execute("SELECT SUM(total_quantity) as c FROM hardware").fetchone()['c'] or 0
    borrowed = db.execute("SELECT SUM(borrowed) as c FROM hardware").fetchone()['c'] or 0
    ending = db.execute("SELECT SUM(available) as c FROM hardware").fetchone()['c'] or 0
    my_borrowed = db.execute("""
...
def profile():
    ...
    stats = db.execute("SELECT COUNT(*) as total, SUM(CASE WHEN status='Pending' THEN 1 ELSE 0 END) as pending FROM transactions WHERE user_id=?", (session['user_id'],)).fetchone()
...
def edit_item(id):
    ...
    db = get_db()
    cur = db.cursor()
    old = db.execute("SELECT * FROM hardware WHERE id=?", (id,)).fetchone()
    if not old:
    ...
    cur.execute("UPDATE hardware SET name=?, category=?, location=?, unit_price=?, total_quantity=?, available=?, borrowed=? WHERE id=?",
...
def export_csv():
    db = get_db()
    hw = db.execute("SELECT * FROM hardware").fetchall()
    si = StringIO()
    w = csv.writer(si)
